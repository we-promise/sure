...
| C12 | Interest, principal, and ending balances are rounded to the account currency precision at their defined output/payment boundary; intermediate daily accrual remains unrounded. | `Loan::AmortizationMathTest` and `Loan::InterestAccrualTest` rounding tests | Required against statement tolerance |
