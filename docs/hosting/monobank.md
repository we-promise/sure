...
- Each sync also re-reads the last few days so unsettled holds stay accurate.
...
settles, monobank may issue the settled record under a different id, so the pending
entry is removed as soon as the hold disappears from the statement and replaced by the
settled one.
...
| `MONOBANK_PENDING_LOOKBACK_DAYS` | `3` | How far back every sync re-reads the statement. |
...
request. Both are normal: the next sync continues where this one stopped. Raise
