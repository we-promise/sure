...
class CreateQuestradeItemsAndAccounts < ActiveRecord::Migration[7.2]
  def change
    ...
    add_index :questrade_items, :status

    # Create provider accounts table (stores individual account data from provider)
    create_table :questrade_accounts, id: :uuid do |t|
      t.references :questrade_item, null: false, foreign_key: true, type: :uuid
    ...
    add_index :questrade_accounts, [ :questrade_item_id, :questrade_account_id ], unique: true
    add_index :questrade_accounts, :questrade_authorization_id
  end
end
