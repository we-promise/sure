class ValuableItemInvoicesController < ApplicationController
  ...
  def destroy
    @lot.invoice.purge
    redirect_to edit_valuable_item_path(@lot), notice: t(".deleted")
  end

  private
    def set_lot
      @lot = ValuableItem.joins(:account).merge(Current.user.accessible_accounts).find(params[:valuable_item_id])
