...
class Settings::ApiKeysController < ApplicationController
  ...
  def create
    ...
    if @api_key.save
      flash[:notice] = t("views.settings.api_keys.api_keys_controller.success")
  ...
  def destroy
    if @api_key.nil?
      flash[:alert] = t("views.settings.api_keys.api_keys_controller.api_key_not_found")
    elsif @api_key.demo_monitoring_key?
      flash[:alert] = t("views.settings.api_keys.api_keys_controller.cannot_be_revoked")
    elsif @api_key.revoke!
      flash[:notice] = t("views.settings.api_keys.api_keys_controller.revoked_successfully")
    else
      flash[:alert] = t("views.settings.api_keys.api_keys_controller.revoke_failed")
    end
    redirect_to settings_api_key_path
  end
