  ...
  def process
    ...
    Account.transaction do
      if account.respond_to?(:trades)
        deleted_count = account.trades.where(source: "traderepublic").delete_all
        Rails.logger.info "TraderepublicAccount::Processor - #{deleted_count} Trade Republic trades for account ##{account.id} deleted before reprocessing."
      end
