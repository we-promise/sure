# The legacy Up route remains usable after its exact connection has migrated.
...
class UpItem::SyncRequest
  ...
  def initialize(item:, actor:)
    unless item.is_a?(UpItem) && item.persisted?
      raise Fence::InvalidSource, "Expected a persisted Up connection"
    end
  ...
  def call
    ApplicationRecord.uncached do
      ...
      Fence.public_send(admission, @item, **options) do
        ApplicationRecord.transaction(requires_new: true) do
          ...
          item = UpItem.where(id: @item_id, family_id: @family_id).lock("FOR UPDATE NOWAIT").first!
    ...
    raise Fence::Busy, "Up synchronization ownership is busy; retry the request", cause: nil
  end

  private
    def routing(lock: false)
      scope = ProviderMigrationControl.where(legacy_type: "UpItem", legacy_id: @item_id).select(*CONTROL_COLUMNS)
      scope = scope.lock("FOR UPDATE NOWAIT") if lock
      control = scope.first
      if control && (control.family_id != @family_id || control.provider_key != "up")
        refuse!
      end
      control&.attributes
    end
    ...
    def verify_native!(control, connection)
      unless connection && connection.id == control.fetch("provider_connection_id") &&
          connection.family_id == @family_id && connection.provider_key == "up" &&
      ...
      unless mappings.one? && mapping.family_id == @family_id && mapping.legacy_type == "UpItem" &&
    ...
    def refuse!
      raise Fence::OwnershipChanged, "Up synchronization owner is unavailable or changed", cause: nil
    end
end
