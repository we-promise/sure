...
class Provider::Traderepublic
    ...
    if response.headers["set-cookie"]
      ...
      set_cookies.each do |cookie|
        if cookie.start_with?("JSESSIONID=")
          ...
          Rails.logger.info "TradeRepublic: JSESSIONID extracted"
          break
        end
      end
    end
  ...
  private
    ...
    def handle_http_response(response)
      Rails.logger.error "TradeRepublic: HTTP response code=#{response.code}, body_size=#{response.body.to_s.bytesize}"
