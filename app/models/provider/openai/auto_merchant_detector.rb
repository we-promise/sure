class Provider::Openai::AutoMerchantDetector
  ...
  attr_reader :client, :model, :transactions, :user_merchants, :custom_provider, :langfuse_trace, :family, :json_mode, :max_tokens

  def initialize(client, model: "", transactions:, user_merchants:, custom_provider: false, langfuse_trace: nil, family: nil, json_mode: nil, max_tokens: nil)
    @client = client
    @model = model
  ...
  private
    ...
    # Also handles providers that don't support json_schema at all (like Cloudflare)
