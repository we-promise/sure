...
module Provider::Plaid::Transactions::CategoryTaxonomy
  CATEGORIES_MAP = {
    income: {
  ...
  def self.resolve(detailed_category)
    return nil if detailed_category.blank?
    key = detailed_category.to_s.downcase.to_sym

    CATEGORIES_MAP.each do |_parent_key, parent_data|
      child = parent_data[:detailed_categories][key]
      next unless child
      return {
