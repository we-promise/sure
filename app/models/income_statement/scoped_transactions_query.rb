...
module IncomeStatement::ScopedTransactionsQuery
  private
    # Refunds, contributions, and loan payments are cash outflows recorded as
    # negative amounts, so they always classify as expense; other negative
    # amounts classify as income.
    def classification_sql(t)
      "CASE WHEN #{t}.refund = true OR #{t}.kind IN ('investment_contribution', 'loan_payment') THEN 'expense' WHEN ae.amount < 0 THEN 'income' ELSE 'expense' END"
    end

    # Entry amount converted to the family currency at the day's exchange
    # rate. Refunds invert the amount, while contribution/loan-payment
    # outflows are flipped positive so they add to expense totals.
    def converted_amount_sql(t)
      "CASE WHEN #{t}.refund = true THEN -ABS(ae.amount * COALESCE(er.rate, 1)) WHEN #{t}.kind IN ('investment_contribution', 'loan_payment') THEN ABS(ae.amount * COALESCE(er.rate, 1)) ELSE ae.amount * COALESCE(er.rate, 1) END"
    end
