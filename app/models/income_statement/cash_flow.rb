...
class IncomeStatement::CashFlow
  ...
  def as_json(*)
    ...
    debt_principal_payments = totals.debt_principal_money.amount
    ...
    result = {
      ...
      # investment/crypto account or paid down a loan's principal. Real cash
      # outflow, but net-worth-neutral, so it's kept out of `spending` /
      # `savings_rate` and reported separately (see Transaction::NON_OPERATING_KINDS).
      investment_contributions: decimal(investment_contributions),
      debt_principal_payments: decimal(debt_principal_payments),
      spending_comparison: {
