...
class Family::DataImporter
  ...
  private
    ...
    def mapped_id(mapping_key, old_id, record_type:, required: true)
      ...
      source_type = MAPPING_TYPES.fetch(mapping_key)
      mapping = @import_session&.source_mappings&.find_by(source_type: source_type, source_id: old_id)
    ...
    def import_accounts(records)
      records.each do |record|
        ...
        unless ACCOUNTABLE_TYPES.include?(accountable_type)
        ...
        if created && data["balance"].present? && !@imported_opening_anchor_account_ids.include?(old_id)
    ...
    def import_transactions(records)
      records.each do |record|
        ...
        if data["tag_ids"].present?
          new_tag_ids = Array(data["tag_ids"]).map do |old_tag_id|
            mapped_id(:tags, old_tag_id, record_type: "Transaction")
          end.compact
        end
