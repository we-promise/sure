class IbkrAccount::ActivitiesProcessor
  ...
  def process
    ...
    {
      trades: trades_count,
      transactions: cash_transactions_count + @fee_transactions_count.to_i
    }
  end
  ...
  private
    ...
    def import_commission_transaction(row, security, date)
      ...
      @fee_transactions_count = @fee_transactions_count.to_i + 1 if result
    end
    ...
    def supported_trade?(row)
      row[:asset_category].to_s == "STK" &&
        ...
        row[:ib_commission].present? &&
        row[:ib_commission_currency].present? &&
