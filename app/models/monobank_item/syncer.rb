class MonobankItem::Syncer
  ...
  # Build a syncer for the given +monobank_item+.
  ...
  def perform_sync(sync)
    ...
    unlinked_accounts = monobank_item.monobank_accounts.needs_setup
    ...
    if linked_accounts.any?
      ...
      raise_if_failed_results!(schedule_results, stage: "Monobank account sync scheduling")

      account_ids = linked_accounts.includes(:account_provider).filter_map { |ua| ua.current_account&.id }
      collect_transaction_stats(sync, account_ids: account_ids, source: "monobank")
    else
      Rails.logger.info "MonobankItem::Syncer - No linked accounts to process"
    end
    ...
    safe_message = I18n.t("monobank_item.errors.sync_failed")
  ...
  # Post-sync hook (no work required for Monobank).
