class Rule::ConditionFilter
  ...
  def label
    I18n.t("rules.conditions.filters.#{key}", default: key.humanize)
  end
  ...
  def operators
    OPERATORS_MAP.dig(type).map do |label, key|
      [ I18n.t("rules.conditions.operators.#{key}", default: label), key ]
    end
  end
