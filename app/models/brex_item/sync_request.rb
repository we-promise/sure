# The legacy Brex route remains usable after its exact connection has migrated.
...
class BrexItem::SyncRequest
  ...
  def call
    ApplicationRecord.uncached do
      ...
      Fence.public_send(admission, @item, **options) do
        ApplicationRecord.transaction(requires_new: true) do
          ...
          item = BrexItem.where(id: @item_id, family_id: @family_id).lock("FOR UPDATE NOWAIT").first!
    ...
    raise Fence::Busy, "Brex synchronization ownership is busy; retry the request", cause: nil
  end

  private
    def routing(lock: false)
      scope = ProviderMigrationControl.where(legacy_type: "BrexItem", legacy_id: @item_id).select(*CONTROL_COLUMNS)
      scope = scope.lock("FOR UPDATE NOWAIT") if lock
      control = scope.first
      if control && (control.family_id != @family_id || control.provider_key != "brex")
        refuse!
      end
      control&.attributes
    end
    ...
    def verify_native!(control, connection)
      unless connection && connection.id == control.fetch("provider_connection_id") &&
          connection.family_id == @family_id && connection.provider_key == "brex" &&
      ...
      unless mappings.one? && mapping.family_id == @family_id && mapping.legacy_type == "BrexItem" &&
    ...
    def refuse!
      raise Fence::OwnershipChanged, "Brex synchronization owner is unavailable or changed", cause: nil
    end
end
