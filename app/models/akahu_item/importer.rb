...
class AkahuItem::Importer
  ...
  private
    ...
    def fetch_accounts_data
      ...
      Rails.logger.error "AkahuItem::Importer - Akahu API error: #{e.message}"
      nil
    rescue JSON::ParserError => e
      Rails.logger.error "AkahuItem::Importer - Failed to parse Akahu API response: #{e.message}"
      nil
    rescue => e
      Rails.logger.error "AkahuItem::Importer - Unexpected error fetching accounts: #{e.class} - #{e.message}"
    ...
    def fetch_pending_transactions_by_account
      ...
      Rails.logger.warn "AkahuItem::Importer - Failed to fetch pending transactions: #{e.message}"
      { success: false, by_account: Hash.new { |hash, key| hash[key] = [] }, error: e.message }
    end
    ...
    def import_transactions(pending_result)
      ...
      akahu_item.akahu_accounts.joins(:account).merge(Account.visible).each do |akahu_account|
        ...
        stats[:failed] += 1
        Rails.logger.error "AkahuItem::Importer - Failed to fetch/store transactions for account #{akahu_account.account_id}: #{e.message}"
      end
    ...
    def fetch_and_store_transactions(akahu_account, pending_transactions, pending_refresh_succeeded:)
      ...
      Rails.logger.info "AkahuItem::Importer - Fetching transactions for account #{akahu_account.account_id} from #{start_date}"
      ...
      Rails.logger.error "AkahuItem::Importer - Akahu API error for account #{akahu_account.id}: #{e.message}"
      { success: false, transactions_count: 0, error: e.message }
    rescue JSON::ParserError => e
      Rails.logger.error "AkahuItem::Importer - Failed to parse transaction response for account #{akahu_account.id}: #{e.message}"
      { success: false, transactions_count: 0, error: "Failed to parse response" }
    rescue => e
      Rails.logger.error "AkahuItem::Importer - Unexpected error fetching transactions for account #{akahu_account.id}: #{e.class} - #{e.message}"
      Rails.logger.error e.backtrace.join("\n")
    ...
    def store_transactions(akahu_account, posted_transactions:, pending_transactions:, replace_pending:)
      ...
      new_posted_transactions = posted_transactions.select do |tx|
        ...
        key.present? && !existing_posted_keys.include?(key)
      end
      ...
      current_pending_transactions = pending_transactions.select do |tx|
        ...
        key = transaction_storage_key(tx.with_indifferent_access)
        key.present? && current_pending_keys.add?(key)
      end
