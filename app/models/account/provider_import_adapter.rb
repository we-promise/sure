class Account::ProviderImportAdapter
  ...
  # @param identity_only [Boolean] Disable heuristic claiming for device-owned source identities
  # @return [Entry] The created or updated entry
  def import_transaction(external_id:, amount:, currency:, date:, name:, source:, category_id: nil, kind: nil, merchant: nil, notes: nil, pending_transaction_id: nil, extra: nil, investment_activity_label: nil, identity_only: false)
    ...
    Account.transaction do
      ...
      if entry.new_record? && !identity_only
      ...
      if entry.new_record? && !incoming_pending
        ...
        if pending_match.nil? && !identity_only
      ...
      if is_new_posted && !identity_only
