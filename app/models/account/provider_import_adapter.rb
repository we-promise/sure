class Account::ProviderImportAdapter
  ...
  # Accepts trade_type for TradeRepublic only (optional)
  def import_trade(security:, quantity:, price:, amount:, currency:, date:, name: nil, external_id: nil, source:, trade_type: nil)
    raise ArgumentError, "security is required" if security.nil?
    raise ArgumentError, "source is required" if source.blank?
