...
export default class extends Controller {
  ...
  render() {
    ...
    const exchange = this.exchangesValue.find(
      (entry) => entry.connection_id === this.selectTarget.value,
    );
    ...
    const connectionFields = Array.isArray(exchange.connection_fields)
      ? exchange.connection_fields
      : [];
    ...
    connectionFields.forEach((field) => {
      ...
      input.className =
        "block w-full rounded-md border border-primary px-3 py-2 text-sm bg-container-inset text-primary placeholder:text-secondary focus:border-primary focus:ring-0";
  ...
  inputTypeFor(key) {
    return /secret|password|token|passphrase|private/i.test(key)
      ? "password"
      : "text";
  }
}
