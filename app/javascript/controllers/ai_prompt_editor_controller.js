...
export default class extends Controller {
  ...
  static values = { resetConfirm: String, statusCustom: String, statusDefault: String }
  ...
  async reset() {
    ...
    const confirmDialog = dialogEl && this.application.getControllerForElementAndIdentifier(dialogEl, "confirm-dialog")
  ...
  selectedPrompt() {
    if (this.hasProviderTarget && this.providerTarget.value) {
      const found = this.promptTargets.find((prompt) => prompt.dataset.key === this.providerTarget.value)
  ...
  updateStatus() {
    ...
    this.statusTarget.textContent = customized ? this.statusCustomValue : this.statusDefaultValue
