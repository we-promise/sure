...
module SslConfigurable
  # Helper methods for SSL initialization and validation.
  # Scoped under SslConfigurable to avoid polluting the global namespace.
...
Rails.application.configure do
  ...
  if ca_file
    ca_file_status = SslConfigurable::InitializerHelper.validate_ca_certificate_file(ca_file)
    ...
    if ca_file_status[:valid]
      combined_path = SslConfigurable::InitializerHelper.create_combined_ca_bundle(ca_file_status[:path])
      if combined_path
  ...
  SslConfigurable::InitializerHelper.log_ssl_configuration(config.x.ssl)
end
