...
class CorsTest < ActionDispatch::IntegrationTest
  ...
  setup do
    @original_allowed_origins = ENV["ALLOWED_ORIGINS"]
    ENV["ALLOWED_ORIGINS"] = ALLOWED_ORIGIN
    # Reset CORS config to pick up the new env var
    Rails.application.middleware.find { |m| m.klass == Rack::Cors }&.tap do
      # Rack::Cors reads config at request time via the initializer lambda
    end
  end

  teardown do
    ENV["ALLOWED_ORIGINS"] = @original_allowed_origins
  end
  ...
  test "cors headers are returned for api endpoints" do
    get "/api/v1/usage", headers: { "Origin" => ALLOWED_ORIGIN }
  ...
  test "cors denies unknown origins" do
    get "/api/v1/usage", headers: { "Origin" => "https://evil.example.com" }
