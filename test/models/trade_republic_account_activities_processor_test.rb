...
class TradeRepublicAccountActivitiesProcessorTest < ActiveSupport::TestCase
  ...
  test "trade names use quantity and instrument without buy or sell wording" do
    Security.create!(ticker: "ISPA", exchange_operating_mic: "XETR", name: "Old security name")
    event = order_execution_detail(
      event_id: "evt_trade_name", quantity: "0.623752", isin: "IE00B3RBWM25", amount: "25.00"
    ).deep_merge(detail: {
    ...
    entry = find_trade("trade_republic_event_evt_trade_name")
    assert_equal "ISPA · 0.623752x STOXX Global Dividend 100 EUR (Dist)", entry.name

    import_event(event)

    assert_equal entry.id, find_trade("trade_republic_event_evt_trade_name").id
    assert_equal "ISPA · 0.623752x STOXX Global Dividend 100 EUR (Dist)", entry.reload.name
  end
  ...
  test "sell names fall back to the ticker without repeating it" do
    ...
    assert_equal "IE00B3RBWM25 · 20x", find_trade("trade_republic_event_evt_ticker_name").name
  end
