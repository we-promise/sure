...
class Loan::VariableRateScheduleTest < ActiveSupport::TestCase
  ...
  test "rate change validation uses the account's first valuation when start date is blank" do
    origination = Date.new(2026, 1, 1)
    loan = build_loan(rate_type: "variable")
    loan.start_date = nil
    loan.account.entries.create!(
      name: "Opening balance", amount: 500_000, currency: "USD",
      date: origination, entryable: Valuation.new(kind: "opening_anchor")
    )
    loan.account.reload

    loan.rate_changes = [ { effective_date: "2025-12-31", rate: "7.25" } ]
    assert_not loan.valid?, "a rate change before the account's first valuation is invalid"
    assert loan.errors.of_kind?(:base, :rate_change_before_origination)

    loan.errors.clear
    loan.rate_changes = [ { effective_date: origination.iso8601, rate: "7.25" } ]
    assert loan.valid?, "a rate change on the account's first valuation is valid"
  end
