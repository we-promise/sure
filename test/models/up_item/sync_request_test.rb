...
class UpItem::SyncRequestTest < ActiveSupport::TestCase
  ...
  test "an unmigrated request schedules only its admitted legacy item" do
    with_source do |item, actor|
      ...
      assert_equal [ "UpItem", item.id ], [ sync.syncable_type, sync.syncable_id ]
  ...
  test "a connection mapping to another legacy item is refused" do
    with_source do |item, actor|
      ...
      ProviderMigrationMapping.create!(family: item.family, provider_migration_control: control,
        role: "connection", legacy_type: "UpItem", legacy_id: SecureRandom.uuid,
  ...
  test "a reparented or missing item does not retarget a previously constructed command" do
    [ :reparented, :missing ].each do |mode|
      with_source do |item, actor|
        command = request(item, actor)
        if mode == :reparented
          UpItem.where(id: item.id).update_all(family_id: create_family.id)
        else
  ...
  private
    def request(item, actor)
      UpItem::SyncRequest.new(item: item, actor: actor)
    end
    ...
    def changing_request(item, actor, &change)
      Class.new(UpItem::SyncRequest) do
    ...
    def with_source
      with_provider_encryption do
        ...
        actor = family.users.create!(email: "up-sync-#{SecureRandom.uuid}@example.com", password: "up-sync-test-password", role: "admin")
        @users << actor
        item = family.up_items.create!(name: "Manual Up sync", access_token: "private-up-token")
        @items << item
        yield item, actor
      ensure
        Sync.where(syncable_type: "ProviderConnection", syncable_id: @connections.map(&:id)).destroy_all
        Sync.where(syncable_type: "UpItem", syncable_id: @items.map(&:id)).destroy_all
        ...
        UpItem.where(id: @items.map(&:id)).delete_all
    ...
    def create_family
      Family.create!(name: "Up manual sync test").tap { |family| @families << family }
    end

    def native_owner(item, state: "active", family: item.family, provider_key: "up", connection: true, mapping: true)
      target = create_provider_connection(family: family, provider_key: provider_key) if connection
      @connections << target if target
      control = ProviderMigrationControl.create!(family: family, provider_key: provider_key, legacy_type: "UpItem",
      ...
      if mapping && target
        ProviderMigrationMapping.create!(family: family, provider_migration_control: control, role: "connection",
          legacy_type: "UpItem", legacy_id: item.id, provider_connection: target)
      end
      control
    end
