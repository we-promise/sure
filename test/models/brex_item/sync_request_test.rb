...
class BrexItem::SyncRequestTest < ActiveSupport::TestCase
  ...
  test "an unmigrated request schedules only its admitted legacy item" do
    with_source do |item, actor|
      ...
      assert_equal [ "BrexItem", item.id ], [ sync.syncable_type, sync.syncable_id ]
  ...
  test "a connection mapping to another legacy item is refused" do
    with_source do |item, actor|
      ...
      ProviderMigrationMapping.create!(family: item.family, provider_migration_control: control,
        role: "connection", legacy_type: "BrexItem", legacy_id: SecureRandom.uuid,
  ...
  test "a reparented or missing item does not retarget a previously constructed command" do
    [ :reparented, :missing ].each do |mode|
      with_source do |item, actor|
        command = request(item, actor)
        if mode == :reparented
          BrexItem.where(id: item.id).update_all(family_id: create_family.id)
        else
          BrexItem.where(id: item.id).delete_all
        end
        assert_refused { command.call }
      end
    end
  ...
  private
    def request(item, actor)
      BrexItem::SyncRequest.new(item: item, actor: actor)
    end
    ...
    def changing_request(item, actor, &change)
      Class.new(BrexItem::SyncRequest) do
    ...
    def with_source
      with_provider_encryption do
        ...
        actor = family.users.create!(email: "brex-sync-#{SecureRandom.uuid}@example.com", password: "brex-sync-test-password", role: "admin")
        @users << actor
        item = family.brex_items.create!(name: "Manual Brex sync", token: "private-brex-token")
        @items << item
        yield item, actor
      ensure
        Sync.where(syncable_type: "ProviderConnection", syncable_id: @connections.map(&:id)).destroy_all
        Sync.where(syncable_type: "BrexItem", syncable_id: @items.map(&:id)).destroy_all
        ...
        BrexItem.where(id: @items.map(&:id)).delete_all
    ...
    def create_family
      Family.create!(name: "Brex manual sync test").tap { |family| @families << family }
    end

    def native_owner(item, state: "active", family: item.family, provider_key: "brex", connection: true, mapping: true)
      target = create_provider_connection(family: family, provider_key: provider_key) if connection
      @connections << target if target
      control = ProviderMigrationControl.create!(family: family, provider_key: provider_key, legacy_type: "BrexItem",
      ...
      if mapping && target
        ProviderMigrationMapping.create!(family: family, provider_migration_control: control, role: "connection",
          legacy_type: "BrexItem", legacy_id: item.id, provider_connection: target)
      end
      control
    end
