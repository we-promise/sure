...
class Provider::CodexTest < ActiveSupport::TestCase
  ...
  test "uses the family Codex PDF prompt and appends extracted PDF text" do
    family = families(:dylan_family)
    family.update!(ai_prompt_codex_pdf: "Extract only checking-account transactions.")
    provider = Provider::Codex.new(command: "/usr/bin/codex")
    provider.stubs(:extract_text).returns("--- Page 1 ---\nCoffee Shop -12.50")

    prompt = provider.send(:prompt_for, "%PDF", [], family: family)

    assert_includes prompt, "Extract only checking-account transactions."
    assert_includes prompt, "Coffee Shop -12.50"
    assert_not_includes prompt, "Do not invent values."
  end

  test "falls back to the built-in Codex PDF prompt" do
    provider = Provider::Codex.new(command: "/usr/bin/codex")
    provider.stubs(:extract_text).returns("Statement text")
