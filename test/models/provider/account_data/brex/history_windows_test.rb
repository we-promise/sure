...
class Provider::AccountData::Brex::HistoryWindowsTest < ActiveSupport::TestCase
  ...
  test "initial hints survive discovery and select independent account windows" do
    with_history_sources([ "2020-01-01", "2023-04-05" ]) do |connection, externals, _accounts, _client, requests|
      ...
      assert_equal [ "2020-01-01", "2023-04-05" ], externals.map { |external| external.reload.metadata.fetch("brex_initial_history_start") }
  ...
  test "an absent hint retains the default from the original Sync clock" do
    with_history_sources([ :absent ]) do |connection, externals, _accounts, _client, requests|
      ...
      refute externals.first.reload.metadata.key?("brex_initial_history_start")
    end
  end
  ...
  test "a completed native checkpoint takes precedence over the original first-read hint" do
    with_history_sources([ "2020-01-01" ]) do |connection, externals, _accounts, _client, requests|
      ...
      assert_equal "2020-01-01", external.reload.metadata.fetch("brex_initial_history_start")
    end
  end
    ...
    [ nil, "", "2026-02-30", "2026-9-01", "2026-09-01T00:00:00Z", " 2026-09-01", 123, {} ].each do |hint|
      with_history_sources([ hint ]) do |connection, externals, accounts, _client, requests|
        error = assert_raises(Provider::AccountData::InvalidResponse, hint.inspect) do
          Provider::AccountData::Brex.new(client: nil, timezone: "UTC").initial_history_start(
            account: { metadata: externals.first.metadata }, observed_at: Time.current)
        end
        assert_equal "Invalid Brex initial history date", error.message
  ...
  test "a hint changed during HTTP leaves captured response evidence and refuses financial publication" do
    with_history_sources([ "2020-01-01" ]) do |connection, externals, accounts, client, requests|
      ...
      client.transaction_response = lambda do |request|
        ExternalAccount.find(external.id).update!(metadata: external.reload.metadata.merge("brex_initial_history_start" => "2021-01-01"))
        [ { id: "after-hint-change", account_id: request.fetch(:account_id), description: "Should remain captured",
          amount: { amount: 1000, currency: "USD" }, initiated_at_date: "2026-09-14T12:00:00Z", posted_at_date: "2026-09-15T12:00:00Z" } ]
      end
      ...
      assert_equal [ "brex_after-hint-change" ], page.records.map { |record| record[:external_id] }
  ...
  private
    class HistoryClient
      ...
      def get_cash_accounts_page(cursor:)
        { items: @ids.map { |id| { id: id, name: "Brex history account", current_balance: { amount: 0, currency: "USD" } } }, next_cursor: nil }
      end
      ...
      def get_cash_transactions_page(account_id, cursor:, start_date:)
        request = { account_id: account_id, cursor: cursor, start: start_date }
        @requests << request
    ...
    def with_history_sources(hints)
      with_provider_encryption do
        connection = create_provider_connection(provider_key: "brex", credentials: { "token" => "private-brex-token" })
        accounts = []
        externals = hints.each_with_index.map do |hint, index|
          metadata = hint == :absent ? {} : { "brex_initial_history_start" => hint }
          external = create_external_account(connection, external_id: "brex-history-#{index}", metadata: metadata)
          account = connection.family.accounts.create!(name: "Brex history #{index}", currency: "USD", balance: 0, accountable: Depository.new)
          accounts << account
    ...
    def assert_windows(requests, _sync, starts)
      ...
      starts.each do |external, start|
        ...
        refute request.key?(:end) # Brex only accepts the original lower bound.
