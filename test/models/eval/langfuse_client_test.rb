...
class Eval::LangfuseClientTest < ActiveSupport::TestCase
  ...
  test "raises when experiment ingestion cannot be flushed" do
    ...
    assert_raises(Eval::Langfuse::Client::ApiError) { client.flush_experiment_items }
  end

  test "buffers experiment items until the batch is explicitly flushed" do
    exporter = OpenTelemetry::SDK::Trace::Export::InMemorySpanExporter.new
    tracing = LangfuseTracing.new(public_key: "pk-test", secret_key: "sk-test", host: "https://langfuse.test",
      processor: OpenTelemetry::SDK::Trace::Export::BatchSpanProcessor.new(exporter, start_thread_on_boot: false))
    LangfuseTracing.stubs(:new).returns(tracing)
    client = Eval::Langfuse::Client.new(public_key: "pk-test", secret_key: "sk-test")

    25.times do |index|
      client.create_experiment_item(name: "eval", input: "input", output: "result", expected_output: "expected",
        experiment_id: "experiment-id", experiment_name: "run", dataset_id: "dataset-id", item_id: "item-#{index}")
    end
    assert_empty exporter.finished_spans

    client.flush_experiment_items

    assert_equal 25, exporter.finished_spans.size
    assert_equal 25, exporter.finished_spans.map(&:span_id).uniq.size
  ensure
    client&.shutdown
  end
