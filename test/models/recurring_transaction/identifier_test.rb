...
class RecurringTransaction::IdentifierTest < ActiveSupport::TestCase
  ...
  test "manual income metadata ignores matching purchase refunds" do
    account = @family.accounts.first
    series = @family.recurring_transactions.create!(
      account: account, name: "PAYCHECK", amount: -100, currency: "USD",
      expected_day_of_month: Date.current.day, last_occurrence_date: 1.month.ago.to_date,
      next_expected_date: Date.current, occurrence_count: 1, status: "active", manual: true)
    account.entries.create!(name: "PAYCHECK", amount: -105, currency: "USD",
      date: Date.current, entryable: Transaction.new(kind: "refund"))
    before = series.attributes.slice("occurrence_count", "last_occurrence_date", "expected_amount_avg")

    @identifier.update_manual_recurring_transactions(6.months.ago)

    assert_equal before, series.reload.attributes.slice(*before.keys)
  end
