...
class OmniauthProviderRegistryTest < ActiveSupport::TestCase
  ...
  test "dynamic OIDC setup uses the custom provider name and redirect URI" do
    ...
    assert registration.options.dig(:extra_authorize_params, :claims).present?
  ...
  test "auth config returns current database providers instead of stale boot providers" do
    ...
    provider = custom_oidc_provider.deep_stringify_keys
    provider["settings"]["default_role"] = "super_admin"
    ProviderLoader.stubs(:load_providers).returns([ provider ])

    providers = AuthConfig.sso_providers

    assert_equal [ "authentik" ], providers.map { |provider| provider[:name] }
    assert_equal "super_admin", providers.first.dig(:settings, :default_role)
  end

  test "dynamic OIDC route supports YAML providers returned by the active provider loader" do
    FeatureFlags.stubs(:db_sso_providers?).returns(true)
    ProviderLoader.stubs(:load_providers).returns([ custom_oidc_provider.deep_stringify_keys ])
  ...
  test "dynamic OIDC route uses YAML providers when no database providers are enabled" do
    Rails.env.stubs(:test?).returns(false)
    FeatureFlags.stubs(:db_sso_providers?).returns(true)
    SsoProvider.stubs(:enabled).returns(SsoProvider.none)
    Rails.application.stubs(:config_for).with(:auth).returns(
      "providers" => [ custom_oidc_provider.deep_stringify_keys ]
    )
