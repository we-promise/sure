  ...
  test "adds a purchase to a physical gold account" do
    assert_difference -> { @account.physical_gold_lots.count }, 1 do
      post physical_gold_lots_path, params: {
        ...
        physical_gold_lot: { description: "Wedding jewelry", acquired_on: Date.current, weight: 10, weight_unit: "gram", karat: 24, cost_amount: 1_000, making_charge: 100, manual_value: 1_100, merchant_id: merchants(:one).id }
      }
    end
