...
class PagesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "dashboard contribution drilldowns use the durable key and eligible accounts" do
    contribution_category = @family.investment_contributions_category
    contribution_category.update!(name: "Long-term investing")
    ...
    assert_response :ok
    donut = css_select("[data-controller='donut-chart']").first
    segment = JSON.parse(donut["data-donut-chart-segments-value"]).find { |item| item["name"] == "Long-term investing" }

    assert_includes segment.fetch("transactions_url"), "q%5Bkinds%5D%5B%5D=investment_contribution"
    assert_includes segment.fetch("transactions_url"), "q%5Baccount_ids%5D%5B%5D=#{@family.accounts.first.id}"
    assert_not_includes segment.fetch("transactions_url"), excluded_account.id.to_s
  end
