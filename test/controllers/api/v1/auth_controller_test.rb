...
class Api::V1::AuthControllerTest < ActionDispatch::IntegrationTest
  ...
  # Duplicate-email creation is now caught earlier by the server-side
  # duplicate guard (see "sso_create_account rejects account creation for
  # an email that already has an account" above), which returns 409 before
  # the user is ever built/validated. This test covers a *different* user
  # validation failure - one the duplicate guard doesn't and shouldn't catch
  # - to keep the general 422-on-invalid-user path covered.
  test "should return 422 when SSO create account fails user validation for a reason other than a duplicate email" do
    linking_code = SecureRandom.urlsafe_base64(32)
    Rails.cache.write("mobile_sso_link:#{linking_code}", {
      provider: "google_oauth2",
      uid: "google-uid-invalid-email",
